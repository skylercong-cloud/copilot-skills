...
页面元素的 `width`、`height`、`max-width`、`max-height` 、`min-height`、`min-width`非必要时**不要设置**，尽量让元素的宽高由内容自然撑开。只有在需要明确限制尺寸（如图片容器、固定宽度卡片）时才写固定值，并用 `r.vw()` 或 `r.resp()` 转换为响应式值而非 raw px。
...
### 11. 设计稿切图必须导出为 2x、100% 画质的 WebP 图片

从设计稿切图时，必须以 **2x（2 倍）尺寸**、**100% 画质**导出，并统一使用 **WebP** 格式。

### 12. 图片和视频禁止使用 `object-fit: cover`

图片或视频元素禁止使用 `object-fit: cover`，因为该属性可能导致媒体内容在苹果设备上显示异常。应根据设计稿与容器尺寸选择不会裁切媒体内容的布局方式。

### 13.设计稿中的图片、视频等静态资源配置

设计稿中的图片、视频，我会在配置里面进行配置。图片重叠的图标和文字我也在配置里面进行配置，而不会把他们作为图片的一部分进行整体切图。你处理时也应该为图片重叠的图标和文字按照设计稿还原布局和样式，而不应把他们默认为图片的一部分

### 14.不要删除/assets/下的css中的/\* scss-kit:managed source=src/styles/xx.scss \*/

它是用来识别该css文件是否使用scss-kit工具生成的标志位，只有有标志位的css文件才允许被该工具编译生成
